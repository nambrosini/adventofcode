...
impl IntCode {
    ...
    fn mem_get(&mut self, mode: Mode, address_offset: i64) -> i64 {
        let addr = match mode {
            ...
            Mode::Pos => self.memory.entry(self.address + address_offset).or_insert(0).clone(),
            Mode::Rel => {
                self.memory.entry(self.address + address_offset).or_insert(0).clone() + self.relative_base
            }
        };
